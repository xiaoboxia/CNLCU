...
import numpy as np
import datetime
from loss import loss_ours_hard
import warnings
...
parser.add_argument('--result_dir', type=str, help='dir to save result txt files', default='/output/results_ours_hard/')
...
parser.add_argument('--time_step', type=int, help='time_step', default=7)
...
def train(train_loader, epoch, model1, optimizer1, model2, optimizer2, before_loss_1, before_loss_2, sn_1, sn_2, noise_or_not):
    ...
    for i, (data, labels, indexes) in enumerate(train_loader):
        ...
        if epoch < args.epoch_decay_start:
            loss_1, loss_2, pure_ratio_1, pure_ratio_2, ind_1_update, ind_2_update, loss_1_, loss_2_ = loss_ours_hard(epoch, before_loss_1[start_point:stop_point], before_loss_2[start_point:stop_point], 
                                                                                                                          sn_1[start_point:stop_point], sn_2[start_point:stop_point], logits1, logits2, labels, rate_schedule[epoch], ind, noise_or_not, co_lambda_plan[epoch], 2.)
        else: 
            loss_1, loss_2, pure_ratio_1, pure_ratio_2, ind_1_update, ind_2_update, loss_1_, loss_2_ = loss_ours_hard(epoch, before_loss_1[start_point:stop_point], before_loss_2[start_point:stop_point], 
                                                                                                                          sn_1[start_point:stop_point], sn_2[start_point:stop_point], logits1, logits2, labels, rate_schedule[epoch], ind, noise_or_not, 0., 2.)
            
            
            
        # print(loss_1, loss_2)
        before_loss_1_list += list(loss_1_)
        before_loss_2_list += list(loss_2_)
...
def main(args):
    ...
    clf1.cuda()
    clf2.cuda()
    optimizer1 = torch.optim.Adam(clf2.parameters(), lr=learning_rate)
